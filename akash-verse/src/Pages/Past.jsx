...
const PastScene = ({ particleRef, textRef }) => {
  useFrame(() => {
  ...
  return (
    <>
      ...
      <directionalLight color="#71ff71" position={[1, 0, 0]} intensity={1} />
      <directionalLight color="#11E8BB" position={[0.75, 1, 0.5]} intensity={1} />
      <directionalLight color="#8200C9" position={[-1, 1, 0.5]} intensity={1} />
    ...
    <div style={{ width: "100vw", height: "100vh", background: "black", position: "relative", overflow: "hidden" }}>
      {story && (
        <div style={{
          ...
          <h1 style={{ fontSize: "1.8rem", marginBottom: "10px" }}>🔮 Past Life Reading for {name}</h1>
          <h3>🌙 Moon Nakshatra: <span style={{ color: "#bbf" }}>{kundli?.moonNakshatra}</span></h3>
          <h3>☄️ Ketu Nakshatra: <span style={{ color: "#fbf" }}>{kundli?.ketuNakshatra}</span></h3>
          <h2 style={{ marginTop: "15px", fontSize: "1.3rem" }}>🌀 Past Life Story</h2>
          <p style={{
            marginTop: "10px",
            lineHeight: "1.6",
            background: "#111",
      ...
      <Canvas camera={{ position: [0, 0, 300], fov: 75 }}>
        <PastScene particleRef={particleRef} textRef={textRef} />
      </Canvas>
    </div>
  );
}
